import { s } from "react-native-wind";
 const NativewindColors ={
    overlay: s`flex-1 rounded-md bg-darkoverlay justify-center pl-1.5`,
    Image_Background: s`flex-1 rounded-lg overflow-hidden`,
    primbtnStyle:s`flex-wrap bg-white p-3 rounded-md w-fit mt-4`,
    secbtnStyle:s`flex-wrap bg-green p-3 rounded-md w-fit mt-4`,
    cardStyle:s`bg-white rounded-lg p-2 m-2`,
    btnWrapper:s`flex-row`,
    cardTitle:s`text-2xl font-bold text-white underline`,
    cardSubTitle:s`text-rose-50 text-sm`,
    cardImage:s`w-40 h-40 rounded-lg`,
}

const bgOverlay = `rgba(0,0,0,0.4)`;
const appGreeen= `#4d9808`;
const appWhite= `#ffffff`;
const appBlack= `#000000`;

export {bgOverlay, NativewindColors,appGreeen,appWhite,appBlack};